...
import { Trophy, Sparkles, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
...
export default function SIH2024Winners({ onViewChange }) {
  ...
  const slides = [
    {
      type: 'image',
      url: '/sih4.png',
      title: 'SIH 2.0 Grand Finale Event',
      desc: 'Prize Distribution and Valedictory Function celebrating extraordinary innovation and technical excellence.'
    },
    {
      type: 'custom',
      title: 'Winners & Podium Finishers',
      desc: 'Honoring the teams that demonstrated outstanding prototypes at SISTec Innovation Hackathon 2.0.'
    }
  ];
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10 text-left">
          <div className="space-y-1.5">
            ...
            <p className="text-brand-gray text-xs sm:text-sm font-normal">
              Here are the winners of SISTec Innovation Hackathon 2.0 (SIH 2024).
            </p>
          </div>
          <button
                  ...
                  {slide.type === 'image' ? (
                    ...
                    <div className="w-full h-full bg-brand-dark/30 flex flex-col justify-center items-center p-6 text-center relative overflow-hidden">
                      ...
                      <div className="relative z-10 max-w-4xl mx-auto space-y-6">
                        ...
                        {/* Top 3 summary cards (Podium) */}
                        <div className="grid grid-cols-3 gap-3 max-w-3xl mx-auto pt-4 items-end">
                          {/* 2nd Place */}
                          <div className="p-3 sm:p-4 rounded-2xl bg-brand-card/60 border border-white/5 flex flex-col items-center justify-between shadow-lg">
                            <span className="text-[10px] font-bold text-slate-300">2ND PLACE</span>
                            <span className="text-xs sm:text-sm font-extrabold text-white block truncate w-full mt-1">Ayush Kumar</span>
                            <span className="text-[10px] sm:text-xs font-bold text-slate-400 mt-0.5">₹20,000</span>
                          </div>

                          {/* 1st Place (Joint Champions) */}
                          <div className="p-4 sm:p-5 rounded-2xl bg-brand-card border border-brand-gold/20 flex flex-col items-center justify-between shadow-2xl relative">
                            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brand-gold text-[8px] text-brand-dark font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">Joint Champions</div>
                            <span className="text-[10px] font-bold text-brand-gold">1ST PLACE</span>
                            <span className="text-[11px] sm:text-xs font-extrabold text-white block truncate w-full mt-1">Harsh & Ankit</span>
                            <span className="text-xs sm:text-sm font-extrabold text-brand-gold mt-0.5">₹25,000 Each</span>
                          </div>
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-brand-gold uppercase tracking-wider whitespace-nowrap">{item.prize}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className="font-mono text-xs font-bold text-brand-blue bg-brand-blue/10 border border-brand-blue/20 px-2.5 py-1 rounded-lg whitespace-nowrap">
                        {item.psCode}
                      </span>
                    </td>
                    <td className="px-6 py-5">
          ...
          {winnersData.map((item, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="font-mono text-[10px] font-bold text-brand-blue bg-brand-blue/10 border border-brand-blue/20 px-2.5 py-1 rounded-lg whitespace-nowrap">
                  {item.psCode}
                </span>
              </div>
